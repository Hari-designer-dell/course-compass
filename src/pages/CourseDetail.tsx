...
import { ArrowLeft, Clock, Users, BookOpen, PlayCircle, CheckCircle2, Bookmark, DollarSign, TrendingUp } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative h-72 md:h-80 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-foreground/80 hover:text-foreground transition-colors text-sm font-medium bg-background/60 backdrop-blur-sm rounded-lg px-3 py-2 border border-border"
          >
          ...
          >
            <Bookmark className={`w-4 h-4 ${isBookmarked(course.id) ? "fill-primary text-primary" : ""}`} />
            {isBookmarked(course.id) ? "Saved" : "Save"}
          </button>
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10 max-w-5xl mx-auto">
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-primary bg-primary/15 backdrop-blur-sm px-3 py-1 rounded-full border border-primary/20 font-mono">
            {course.subject}
          </span>
          <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground mt-3 leading-tight">
            {course.course_title}
          </h1>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              <h2 className="font-display text-2xl font-bold text-foreground mb-3">About this course</h2>
              ...
              <div className="mt-4 flex items-center gap-2 bg-card rounded-lg border border-border p-3">
                <TrendingUp className="w-4 h-4 text-primary" />
                <span className="text-sm font-bold text-foreground">
                  Popularity Score: <span className="text-primary">{popScore.toFixed(0)}</span>
                </span>
                <span className="text-xs text-muted-foreground ml-2 font-mono">(0.6 × subscribers + 0.4 × reviews)</span>
              </div>
            </motion.section>

            <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="font-display text-2xl font-bold text-foreground mb-1">Curriculum</h2>
              <p className="text-muted-foreground text-sm mb-5 font-mono">{curriculum.length} modules · {totalLessons} lessons · {course.content_duration}h</p>
              <div className="space-y-3">
                {curriculum.map((module, mi) => (
                  <div key={mi} className="bg-card rounded-xl border border-border overflow-hidden shadow-card hover:border-primary/20 transition-colors">
                    <div className="flex items-center gap-3 px-5 py-4">
                      <span className="w-8 h-8 rounded-lg bg-primary/15 text-primary flex items-center justify-center text-sm font-bold flex-shrink-0 border border-primary/20">
                        {mi + 1}
                      </span>
                      <h3 className="font-bold text-foreground text-sm">{module.title}</h3>
                      <span className="ml-auto text-xs text-muted-foreground font-mono">{module.lessons.length} lessons</span>
                    </div>
                    <div className="border-t border-border px-5 py-3 space-y-2">
                      {module.lessons.map((lesson, li) => (
                        <div key={li} className="flex items-center gap-3 text-sm text-muted-foreground py-1 hover:text-foreground transition-colors">
                          <PlayCircle className="w-4 h-4 flex-shrink-0 text-primary/50" />
                          {lesson}
                        </div>
                      ))}
                    </div>
            ...
            <div className="sticky top-24 bg-card rounded-xl border border-border shadow-card p-6 space-y-5">
              ...
              <div className="border-t border-border pt-5">
                <div className="mb-4 text-center">
                  <span className="text-sm font-bold text-muted-foreground">{course.level}</span>
                </div>
                ...
                <motion.button
                  ...
                  className={`w-full py-3.5 rounded-xl font-bold text-base transition-all flex items-center justify-center gap-2 ${
                    enrolled
                      ...
                      : "bg-gradient-warm text-primary-foreground shadow-elevated hover:shadow-glow"
                  }`}
                >
                  {enrolled ? (
                    <>
              ...
              <p className="text-xs text-muted-foreground text-center font-mono">Full lifetime access · Certificate of completion</p>
            </div>
          </motion.aside>
        </div>
      </main>
